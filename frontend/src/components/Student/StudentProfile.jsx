...
const StudentProfile = () => {
  ...
  const addItem = (field, value) => {
    ...
    // Reset the input
    const inputType = field.includes('.') ? field.split('.')[1] : field;
      ...
      case 'social':
        return (
          <>
            ...
            <div className="bg-white p-5 rounded shadow-sm">
              ...
              <div className="grid grid-cols-1 gap-3">
                <div>
                  ...
                  <input type="url" name="social_links.github" value={formData.social_links.github} onChange={handleChange} 
                    className="w-full p-2 border rounded" placeholder="https://github.com/username" />
                </div>
                <div>
                  <label className="block text-sm mb-1">LinkedIn</label>
                  <input type="url" name="social_links.linkedin" value={formData.social_links.linkedin} onChange={handleChange} 
                    className="w-full p-2 border rounded" placeholder="https://linkedin.com/in/username" />
                </div>
                <div>
                  <label className="block text-sm mb-1">Portfolio</label>
                  <input type="url" name="social_links.portfolio" value={formData.social_links.portfolio} onChange={handleChange} 
                    className="w-full p-2 border rounded" placeholder="https://yourportfolio.com" />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-5 rounded shadow-sm mt-4">
              <h2 className="text-xl font-semibold mb-3 flex items-center gap-2">
                <Users className="text-blue-600" /> Mentorship & Availability
              </h2>
              <div className="mb-4">
                <label className="flex items-center mb-2">
                  ...
                  />
                  <span>I'm looking for a mentor</span>
                </label>
                
                {formData.mentorship_interests.seeking_mentor && (
                  <div className="mt-2">
                    <label className="block text-sm font-medium mb-2">Topics I'd like mentorship in:</label>
                    <div className="flex flex-wrap gap-2 mb-2">
                      {formData.mentorship_interests.mentor_topics.map((topic, index) => (
                        <div key={index} className="bg-indigo-100 text-indigo-700 px-2 py-1 rounded-full flex items-center gap-1">
                          {topic}
                          
                          <button type="button" onClick={() => removeItem('mentorship_interests.mentor_topics', index)} className="text-indigo-700">
                          <X size={14} />
                        </button>
                      </div>
                    ))}
                  ...
                  <div className="flex">
                    <input type="text" value={newItem.mentorTopic} onChange={(e) => setNewItem({...newItem, mentorTopic: e.target.value})}
                      className="flex-1 p-2 border rounded-l" placeholder="Add topic for mentorship" />
            ...
            <div>
              <h3 className="text-md font-medium mb-2 flex items-center gap-1">
                <Clock size={16} /> Preferred Working Hours
              </h3>
  ...
  <div className="max-w-4xl mx-auto p-4 bg-gray-50">
    ...
    <div className="flex flex-wrap gap-2 mb-6 justify-center">
      ...
      <button 
        ...
        className={`px-4 py-2 rounded-lg ${activeTab === 'social' 
          ? 'bg-blue-600 text-white' 
          : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
      >
        Social & Mentorship
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="sticky bottom-0 bg-white p-4 border-t mt-6 -mx-4 flex justify-end">
        <button 
          type="submit" 
          disabled={saving}
          className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-lg shadow-md"
        >
          {saving ? 'Saving...' : (
            <>
              <Save size={18} />
