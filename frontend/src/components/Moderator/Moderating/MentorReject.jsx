...
const MentorReject = () => {
  ...
  useEffect(() => {
    const fetchRejectedMentors = async () => {
      try {
        setLoading(true);
        const response = await axios.get('http://localhost:4000/api/admin/mentors?status=rejected');
  ...
  const handleRestoreMentor = async () => {
    ...
    try {
      await axios.put(`http://localhost:4000/api/admin/mentors/${mentorToRestore._id}/restore`);
